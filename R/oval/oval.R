...
exclude({
  ...
  ovalPCA <- prcomp(oval[, 2:12], center=TRUE, scale=TRUE)
  summary(ovalPCA)
  print(ovalPCA)
  ...
  fit2 <- lm('price~0+carat+color+clarity+lengthWidth', data=oval)
  summary(fit2)
...
fit3 <- lm('price~0+carat+color+clarity+lengthWidth', data=subsetOval)
summary(fit3)
...
model <- predict(fit3, data=subsetOval)
